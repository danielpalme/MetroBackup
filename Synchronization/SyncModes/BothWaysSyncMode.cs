...
{
    /// <summary>
    /// <see cref="ISyncMode"/> that performs synchronization in both directions.
    /// </summary>
    public class BothWaysSyncMode : ISyncMode
    {
        /// <summary>
        ...
        public string Name
        {
            get
            {
                return Common.SyncModeType_BothWays;
            }
        }
        ...
        {
            if (!File.Exists(fileOnReferenceFolder)
                || File.GetLastWriteTime(fileOnTargetFolder) > File.GetLastWriteTime(fileOnReferenceFolder).AddSeconds(1))
            {
                return new CopyFileSyncItem(fileOnTargetFolder, fileOnReferenceFolder);
            }
            else
            {
                return null;
        ...
        {
            if (!Directory.Exists(directoryInReferenceFolder))
            {
                return new CreateDirectorySyncItem(directoryInReferenceFolder);
            }
            else
            {
                return null;
